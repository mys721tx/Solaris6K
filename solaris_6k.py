...
def get_mean_anomaly(mean_longitude, perihelion_longitude, additional_terms, time):
    """
...
def get_ecliptic_coordinate(
    heliocentric_coordinate, perihelion_argument, ascending_node_longitude, inclination
):
    """
    ...
    ecliptic_x_matrix = np.column_stack(
        (
            ...
            * cos_wrapper(inclination),
            np.array(
                [0 for num in enumerate(perihelion_argument)], np.dtype("float64")
            ),
        )
    )
...
def main():
    ...
    semi_major_axes = get_current_value(
        constants.SEMI_MAJOR_AXES, constants.SEMI_MAJOR_AXES_RATE, century_since_epoch
    )

    eccentricities = get_current_value(
        constants.ECCENTRICITIES, constants.ECCENTRICITIES_RATE, century_since_epoch
    )

    inclinations = get_current_value(
        constants.INCLINATIONS, constants.INCLINATIONS_RATE, century_since_epoch
    )

    mean_longitudes = get_current_value(
        constants.MEAN_LONGITUDES, constants.MEAN_LONGITUDES_RATE, century_since_epoch
    )
    ...
    mean_anomalies = get_mean_anomaly(
        ...
        (constants.TERM_B, constants.TERM_C, constants.TERM_S, constants.TERM_F),
        century_since_epoch,
    )
    ...
    heliocentric_coordinates = np.column_stack(
        (
            ...
            * sin_wrapper(eccentric_anomalies),
            np.array(
                [0 for num in enumerate(mean_anomalies)], np.dtype("float64")
            ),
        )
    )
    ...
    eq_matrix = np.matrix(
        [
            [1, 0, 0],
            [0, cos_wrapper(constants.OBLIQUITY), -sin_wrapper(constants.OBLIQUITY)],
            [0, sin_wrapper(constants.OBLIQUITY), cos_wrapper(constants.OBLIQUITY)],
        ]
    )
